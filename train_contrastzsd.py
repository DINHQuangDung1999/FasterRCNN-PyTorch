...
from model.faster_rcnn_zsd_contrastZSD import FasterRCNN
...
def train(args):
    args = EasyDict({'config_path': 'config/dior_zsd_resnet_contrastzsd.yaml',
                     'checkpoint': None,
                    # 'checkpoint': '/home/qdinh/FasterRCNN-PyTorch/checkpoints/DIOR/frcnn_descreg_0.pt'
                     })
    ...
    lmda = model_config['lambda']
    for epoch_num in range(start_epoch, num_epochs):
        rpn_classification_losses = []
        rpn_localization_losses = []
        frcnn_seen_classification_losses = []
        frcnn_unseen_classification_losses = []
        frcnn_region_contrastive_losses = []
        ...
        # break 
            ...
            frcnn_loss = frcnn_output['frcnn_localization_loss'] + frcnn_output['seen_classification_loss'] \
                            + lmda*frcnn_output['unseen_classification_loss'] + beta*frcnn_output['region_contrastive_loss']
            ...
            frcnn_seen_classification_losses.append(frcnn_output['seen_classification_loss'].item())
            frcnn_unseen_classification_losses.append(lmda*frcnn_output['unseen_classification_loss'].item())
            frcnn_region_contrastive_losses.append(beta*frcnn_output['region_contrastive_loss'].item())

            if     np.isnan(rpn_output['rpn_classification_loss'].item()) \
                or np.isnan(rpn_output['rpn_localization_loss'].item()) \
                or np.isnan(frcnn_output['seen_classification_loss'].item()) \
                or np.isnan(frcnn_output['unseen_classification_loss'].item()) \
                or np.isnan(frcnn_output['frcnn_localization_loss'].item())\
                or np.isnan(frcnn_output['region_contrastive_loss'].item()):
                breakpoint()

            if iter_num % 1 == 0:
                loss_output = 'Epoch {} | Iter {}'.format(epoch_num, iter_num)
                loss_output += ' | RPN Cls: {:.4f}'.format(np.mean(rpn_classification_losses))
                loss_output += ' | RPN Reg: {:.4f}'.format(np.mean(rpn_localization_losses))
                loss_output += ' | Lcls_s: {:.4f}'.format(np.mean(frcnn_seen_classification_losses))
                loss_output += ' | Lcls_u: {:.4f}'.format(np.mean(frcnn_unseen_classification_losses))
                loss_output += ' | Lcon_r: {:.4f}'.format(np.mean(frcnn_region_contrastive_losses))
                loss_output += ' | Lreg: {:.4f}'.format(np.mean(frcnn_localization_losses))
                print(loss_output)
        ...
        checkpoint_dir = os.path.join(train_config['ckpt_path'], train_config['ckpt_name'] + f'_contrastzsd_{epoch_num}.pt')
