...
from model.faster_rcnn_dpif import FasterRCNN
...
def train(args):
    args = EasyDict({'config_path': 'config/dior_zsd_resnet_dpif.yaml',
                    #  'checkpoint': None,
                     'checkpoint': '/home/qdinh/FasterRCNN-PyTorch/checkpoints/DIOR/frcnn_dpif_0.pt',
    ...
    semantic_embeddings = get_embeddings(dataset_config['dataset_name'], 'w2v')
    import json
    split = open('/home/qdinh/data/DIOR/categories_dior_16_4.json')
    split = json.load(split)
    seen_classes, unseen_classes = split['seen'], split['unseen']
    seen_classes = ['__background__'] + seen_classes
    cls2asso = open('/home/qdinh/data/DIOR/cls2asso_dior_w2v_dist_5.json')
    cls2asso = json.load(cls2asso)
    
    faster_rcnn_model = FasterRCNN(model_config=model_config, 
    ...
    for k, v in faster_rcnn_model.named_parameters():
        if 'roi_head.cls_layer' not in k:
    ...
    for epoch_num in range(start_epoch, num_epochs):
        ...
        #     loss = frcnn_output['frcnn_classification_loss'] + frcnn_output['frcnn_association_loss']

        #     frcnn_classification_losses.append(frcnn_output['frcnn_classification_loss'].item())
        #     frcnn_association_losses.append(frcnn_output['frcnn_association_loss'].item())

        #     if iter_num % 20 == 0:
        #         loss_output = 'Epoch {} | Iter {}'.format(epoch_num, iter_num)
        #         loss_output += ' | FRCNN Cls Loss : {:.4f}'.format(np.mean(frcnn_classification_losses))
        #         loss_output += ' | FRCNN association Loss : {:.4f}'.format(np.mean(frcnn_association_losses))
        #         print(loss_output)
        ...
        # checkpoint_dir = os.path.join(train_config['ckpt_path'], train_config['ckpt_name'] + f'_dpif_{epoch_num}.pt')
        # torch.save(faster_rcnn_model.state_dict(), checkpoint_dir)
        # print('Done Training...')
        
        # infer images at each epochs
        os.makedirs(f'./dump/DIOR/dpif/seen/epoch{epoch_num}', exist_ok=True)
        infer(faster_rcnn_model.eval(), dataset_testseen, 'trad', 0.6, n_images = 20, dump_dir = f'./dump/DIOR/dpif/seen/epoch{epoch_num}')
        os.makedirs(f'./dump/DIOR/dpif/unseen/epoch{epoch_num}', exist_ok=True)
        infer(faster_rcnn_model.eval(), dataset_testunseen, 'zsd', 0.1, n_images = 20, dump_dir = f'./dump/DIOR/dpif/unseen/epoch{epoch_num}')
        faster_rcnn_model.train()
        # # Evaluation
