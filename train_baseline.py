...
from model.faster_rcnn_zsd_baseline import FasterRCNN
...
def train(args):
    args = EasyDict({'config_path': 'config/dior_zsd_resnet_baseline.yaml',
                     'checkpoint': None,
                    #  'checkpoint': '/home/qdinh/FasterRCNN-PyTorch/checkpoints/DIOR/frcnn_baseline_0.pt',
    ...
    for epoch_num in range(start_epoch, num_epochs):
        ...
        # break 
            ...
            loss = frcnn_output['frcnn_classification_loss']
        ...
        checkpoint_dir = os.path.join(train_config['ckpt_path'], train_config['ckpt_name'] + f'_baseline_{epoch_num}.pt')
        torch.save(faster_rcnn_model.state_dict(), checkpoint_dir)
        print('Done Training...')
        
        # infer images at each epochs
        os.makedirs(f'./dump/DIOR/baseline/seen/epoch{epoch_num}', exist_ok=True)
        infer(faster_rcnn_model.eval(), dataset_testseen, 'trad', 0.6, n_images = 20, dump_dir = f'./dump/DIOR/baseline/seen/epoch{epoch_num}')
        os.makedirs(f'./dump/DIOR/baseline/unseen/epoch{epoch_num}', exist_ok=True)
        infer(faster_rcnn_model.eval(), dataset_testunseen, 'zsd', 0.1, n_images = 20, dump_dir = f'./dump/DIOR/baseline/unseen/epoch{epoch_num}')
        faster_rcnn_model.train()
        # # Evaluation
