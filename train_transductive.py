...
from model.faster_rcnn_zsd_transductive import FasterRCNN
...
def train(args):
    ...
    if model_config['style'] == 'zsd':
        semantic_embedding = get_embeddings(dataset_config['dataset_name'], 'bert')
    ...
    for epoch_num in range(start_epoch, num_epochs):
        ...
        checkpoint_dir = os.path.join(train_config['ckpt_path'], train_config['ckpt_name'] + f'descreg_{epoch_num}.pt')
        ...
        if epoch_num % train_config['eval_every'] == 0 or epoch_num == num_epochs - 1:
            ...
            predict_coco(faster_rcnn_model, dataset_test, pred_style = 'zsd')
