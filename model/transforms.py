...
def crop(image, target, region):
    ...
    if "bboxes" in target:
        ...
        target["bboxes"] = cropped_boxes.reshape(-1, 4)
        target["area"] = area
    ...
    if "bboxes" in target or "masks" in target:
        # favor boxes selection when defining which elements to keep
        # this is compatible with previous implementation
        if "bboxes" in target:
            cropped_boxes = target["bboxes"].reshape(-1, 2, 2)
...
def get_transforms(training):
    ...
    scales = [400, 416, 432, 448, 464, 480, 512, 544, 576, 608, 640, 672, 704, 736, 768, 800]
