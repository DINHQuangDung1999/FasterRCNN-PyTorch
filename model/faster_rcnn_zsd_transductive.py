...
class ROIHead(nn.Module):
    ...
    def __init__(self, model_config, num_classes, num_seen_classes, in_channels, semantic_embedding = None, emb_dim = None):
        ...
        self.fc7 = nn.Linear(self.fc_inner_dim, self.fc_inner_dim, bias = False)
        ...
        if self.style == 'zsd':
            ...
            self.visual_projection = nn.Linear(self.fc_inner_dim, self.emb_dim, bias = False)
    ...
    def forward(self, feat, proposals, image_shape, target, pred_style):
        ...
        if self.style == 'zsd':
            ...
            projected_visual_feats = self.visual_projection(box_fc_7) #p_s
            normed_projected_visual_feats = F.normalize(projected_visual_feats, dim = -1)
            # normed_projected_emb_feats = F.normalize(projected_emb_feats, dim = -1)
            cls_scores = normed_projected_visual_feats @ self.S.T
        ...
        if self.training and target is not None:
            if self.style == 'zsd':
                scores_softmax = F.softmax(cls_scores, dim = 1)
                seen_softmax = scores_softmax[:,:self.n_seen]
                unseen_softmax = scores_softmax[:,self.n_seen:]
                # seen loss 
                seen_loss = F.nll_loss(torch.log(seen_softmax), labels)/self.n_seen
                frcnn_output['seen_classification_loss'] = seen_loss
                # unseen loss 
                

                unseen_loss = -torch.log(torch.sum(scores_softmax[:,self.n_seen:], dim = 1))/(self.num_classes - self.n_seen)
                frcnn_output['unseen_classification_loss'] = unseen_loss
                # regularization term
                breakpoint()
                reg1 = torch.norm(self.fc7.weight, 2)
                reg2 = torch.norm(self.visual_projection.weight, 2)
                
                frcnn_output['regularization_loss'] = unseen_loss
